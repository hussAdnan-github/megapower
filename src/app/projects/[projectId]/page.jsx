    ...
    <>
      ...
      <header className="bg-blue-600 text-white text-center py-20 px-5">
        ...
        <div className="flex flex-wrap justify-center gap-4 text-sm mt-4">
          <span className="flex items-center gap-2"><i className="fas fa-map-marker-alt"></i> الموقع: {project.location}</span>
          <span className="flex items-center gap-2"><i className="fas fa-tag"></i> الفئة: {project.category}</span>
          <span className="flex items-center gap-2"><i className="fas fa-calendar-alt"></i> تم الانتهاء: {project.completed}</span>
        </div>
      </header>
            ...
            {project.images.map((img, index) => (
              <Image
                ...
                className={`cursor-pointer rounded-lg border-2 transition-transform duration-300 transform hover:scale-105 ${
                  activeImage === img ? 'border-blue-600 shadow-md' : 'border-transparent'
                }`}
